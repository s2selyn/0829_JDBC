...
public class MemberController {
	...
	public int save(String userId, String userPwd, String userName, String email) {
		...
		// DAO로 넘어감
		// 갔다오니 뭐가 옴, 뭔지 모르지만 1아니면 0, 받아서 변수에 받아줌, int형 변수 result에 담아줌
		int result = new MemberDao().save(member);
		// 컨트롤러는 가지고 있는다고 의미없음, 처음에 뷰가 DAO못가니까 불러서 여기 왔음, 컨트롤러도 얘가한게 아니고 DAO한테 해달라고 해서 갔다왔음
		// 됐을지 안됐는지 모르지만 보여줘야함 --> 뷰한테 돌려줘야함, 뭘 돌려줄진 모르지만 리턴에 결과를 돌려줌
		// 아까 뭘 돌려줄지 몰라서 메소드 반환타입 void로 해놓은거 int로 수정 
		// 2. 요청 처리 후 결과값 반환
		return result;
		
		// 컨트롤러 할일 끝, 뷰가 불러서 왔으니까 뷰로 다시감
